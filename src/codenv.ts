...
type ProfileType = "codex" | "claude";
type DefaultProfiles = Partial<Record<ProfileType, string>>;
...
const DEFAULT_PROFILE_TYPES: ProfileType[] = ["codex", "claude"];
const DEFAULT_UNSET_KEYS: Record<ProfileType, string[]> = {
...
function normalizeType(value: string | null | undefined): ProfileType | null {
...
function hasTypePrefix(name: string, type: ProfileType) {
...
function hasEnvKeyPrefix(profile: Profile | undefined, prefix: string) {
...
function shouldRemoveCodexAuth(
  profileName: string,
  profile: Profile | undefined,
  requestedType: ProfileType | null
) {
...
function stripTypePrefixFromName(name: string, type: string) {
  if (!name) return name;
...
function findProfileKeysByName(
  config: Config,
  name: string,
...
function generateProfileKey(config: Config) {
  const profiles = config && config.profiles ? config.profiles : {};
  for (let i = 0; i < 10; i++) {
...
function normalizeShell(value: string | null | undefined) {
  if (!value) return null;
  const raw = String(value).trim().toLowerCase();
  if (!raw) return null;
...
function detectShell(explicitShell: string | null | undefined) {
...
function getShellRcPath(shellName: string | null) {
...
function getShellSnippet(shellName: string | null) {
  if (shellName === "fish") {
    return [
      "function codenv",
...
function upsertShellSnippet(rcPath: string, snippet: string) {
...
function profileMatchesType(profile: Profile | undefined, type: ProfileType) {
  if (!profile) return false;
...
function getTypeDefaultUnsetKeys(type: ProfileType) {
  return DEFAULT_UNSET_KEYS[type] || [];
}

function getFilteredUnsetKeys(config: Config, activeType: ProfileType | null) {
...
function resolvePath(p: string | null | undefined) {
  if (!p) return null;
  if (p.startsWith("~")) {
...
function findConfigPath(explicitPath: string | null) {
  if (explicitPath) {
...
function findConfigPathForWrite(explicitPath: string | null) {
...
function readConfigIfExists(configPath: string | null): Config {
...
function shellEscape(value: string | number | boolean) {
  const str = String(value);
  return `'${str.replace(/'/g, `'\\''`)}'`;
}

function expandEnv(input: string | null | undefined) {
...
function ask(rl: readline.Interface, question: string) {
  return new Promise((resolve) => {
    rl.question(question, (answer) => resolve(answer));
  });
}

async function askRequired(rl: readline.Interface, question: string) {
  while (true) {
...
async function askConfirm(rl: readline.Interface, question: string) {
  const answer = String(await ask(rl, question)).trim().toLowerCase();
  return answer === "y" || answer === "yes";
}

async function askType(rl: readline.Interface): Promise<ProfileType> {
  while (true) {
    const answer = String(
...
async function runInteractiveAdd(configPath: string) {
...
function addConfig(config: Config, addArgs: ReturnType<typeof parseAddArgs>) {
  if (!config.profiles || typeof config.profiles !== "object") {
    config.profiles = {};
  }
...
function buildListRows(config: Config): ListRow[] {
  const profiles = config && config.profiles ? config.profiles : {};
...
function printList(config: Config) {
  ...
  const formatRow = (name, type, note) =>
...
async function runInteractiveUse(config: Config) {
...
function printShow(config: Config, profileName: string) {
...
function printUnset(config: Config) {
...
function resolveProfileName(config: Config, params: string[]) {
...
function getDefaultProfiles(config: Config): DefaultProfiles {
...
function deleteDefaultProfileEntry(config: Config, type: ProfileType) {
  if (!config.defaultProfiles || typeof config.defaultProfiles !== "object") {
    return false;
  }
  let changed = false;
...
function resolveDefaultProfileForType(
  config: Config,
  type: ProfileType,
  value: string
) {
...
function getResolvedDefaultProfileKeys(config: Config): DefaultProfiles {
...
function isEnvValueUnset(value: EnvValue) {
  return value === null || value === undefined || value === "";
}

function buildEffectiveEnv(profile: Profile | undefined, activeType: ProfileType | null) {
...
function envMatchesProfile(profile: Profile | undefined) {
  if (!profile || !profile.env) return false;
  for (const key of Object.keys(profile.env)) {
